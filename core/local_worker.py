...
class Local_Worker(Worker):
    ...
    def run(self, env=None):
        ...
        self.agent.reset()
        if env.gui == 0:
            ep = tqdm(range(env.episode_count), desc="Episode processing ")
        else:
            ep = range(env.episode_count)
        try:
            for e in ep:
                self.agent.reset()
                ...
                for t in dat:
                    ...
                    if env.manage_date() == 1 or terminal is True:
                        env.lst_reward_daily.append(env.reward['daily'])
                        env.wallet.episode_process()
                        '''
                ...
                env.wallet.historic_process()
                env.logger._add("######################################################", self.name)
                env.logger._add("Total reward : " + str(env.reward['total']), self.name)
                env.logger._add("Average daily reward : " + str('{:.3f}'.format(env.avg_reward(env.lst_reward_daily, 0))), self.name)
                ...
                env.logger._add("Sharp ratio : " + str('{:.3f}'.format(env.wallet.historic['sharp_ratio'][env.current_step['episode'] - 1])), self.name)
                env.logger._add("Mean return : " + str('{:.3f}'.format(env.wallet.historic['mean_return'][env.current_step['episode'] - 1])), self.name)
                env.logger._add("Max Drawdown : " + str('{:.3f}'.format(env.wallet.historic['max_drawdown'][env.current_step['episode'] - 1])), self.name)
                env.logger._add("Max return : " + str('{:.3f}'.format(env.wallet.historic['max_return'][env.current_step['episode'] - 1])), self.name)
                ...
                env.logger._add("Step : " + str(env.current_step['step']), self.name)
                env.logger._add("######################################################", self.name)
