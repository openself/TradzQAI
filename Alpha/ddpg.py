...
from keras.layers import Activation, concatenate, multiply, PReLU, add
...
class DDPG(object):
    ...
    def build_critic(self):
        ...
        # output layer
        model = multiply([upper_model, action])
        model = Dense(1)(model)
        return model
    ...
    def build_actor(self):
        ...
        """
        # lower layer
        lower_model = [self.build_network(self.model_config['actor_lower'], input_shape=(self.history_length, self.n_stock, 1)) for _ in range(1  + self.n_smooth + self.n_down)]
        merged = concatenate(lower_model)
        # upper layer
    ...
    def build_network(self, conf, model=None, input_shape=None, is_conv=True):
        ...
        if _model is None:
            ...
            model = Lambda(lambda x: x,  input_shape=input_shape)(model)
        else:
            model = _model
            
        for x in conf:
            if x['is_drop']:
                model = Dropout(x['drop_rate'])(model)
            if x['type'] is 'full':
                if is_conv:
                    model = Flatten()(model)
                    is_conv = False
                model = Dense(x['n_feature'])(model)
            elif x['type'] is 'conv':
                model = Convolution2D(nb_filter=x['n_feature'], nb_row=x['kw'], nb_col=1, border_mode='same')(model)
                is_conv=True
            if x['is_batch']:
                if x['type'] is 'full':
                ...
                if x['type'] is 'conv':
                    model = BatchNormalization(axis=1)(model)
            model = Activation(x['activation'])(model)
        return model
    ...
    def transform_input(self, input):
        ...
        for n_sm in range(2, self.n_smooth + 2):
            smoothed.append(
                tf.reduce_mean(tf.stack([input[:, self.n_history - st - self.history_length:self.n_history - st, :, :] for st in range(n_sm)]),0))
        # downsample data
        down = []
        for n_dw in range(2, self.n_down + 2):
            sampled_ = tf.stack([input[:, idx, :, :] for idx in range(self.n_history-n_dw*self.history_length, self.n_history, n_dw)])
